...
commands.add({
    ...
    run: async({ sius, m, args, Func, dl }) => {
        ...
        if (!text) return m.reply("⚠️ Harap masukkan pertanyaan atau perintah untuk levi.")
        const zer = await dl.googleImage("levi-ackerman")
        sius.levi = sius.levi || {}
        let chatHistory = sius.levi
        ...
        const historyText = chatHistory[m.sender]
            ...
            .join("\n");
        const prompt = `kamu adalah captain levi ackerman dari attack on titan. kamu dingin, to the point, dan punya standar tinggi. kamu tidak suka basa-basi dan selalu fokus pada efisiensi. gaya bicaramu pendek-pendek, tegas, dan kadang sinis. kamu benci kotor dan sangat disiplin.`;
