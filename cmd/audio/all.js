...
commands.add({
    run: async({ 
        ...
        try {
            ...
            let isAudio = q.type === "audioMessage" || /audio/.test(mime)            
            if (!isAudio) return m.reply("*Reply audio/pesan suara yang mau dikasih efek!*")            
            let inputPath = join(__dirname, `../../lib/database/sampah/input_${Date.now()}.mp3`)
            let outputPath = join(__dirname, `../../lib/database/sampah/output_${Date.now()}.mp3`)
            ...
            let set = effects[command.toString()]
            let ffmpegCmd = `ffmpeg -i ${inputPath} ${set} ${outputPath}`            
            exec(ffmpegCmd, async (err) => {
                unlinkSync(inputPath)
                if (err) return m.reply("Gagal!")  
                let audio = readFileSync(outputPath)
                unlinkSync(outputPath)                
                await m.reply({ audio, mimetype: "audio/mpeg", ptt: false })
            })
        } catch (e) {
            sius.cantLoad(e)
        }
    },
    name: ["bass", "blown", "deep", "earrape", "fast", "fat", "nightcore", "reverse", "robot", "slow", "smooth", "tupai", "squirrel", "chipmunk", "audio8d", "echo", "distortion", "pitch", "reverb", "flanger", "apulsator", "tremolo", "chorus"],
    command: ["bass", "blown", "deep", "earrape", "fast", "fat", "nightcore", "reverse", "robot", "slow", "smooth", "tupai", "squirrel", "chipmunk", "audio8d", "echo", "distortion", "pitch", "reverb", "flanger", "apulsator", "tremolo", "chorus"],
    category: "audio",
    limited: true
})