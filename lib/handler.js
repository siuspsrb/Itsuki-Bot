...
export async function handler(sius, m, msg, store, groupCache) {
    ...
    sius.sewaHandling = (type, data) => {
        if (!type) return null
        // GET
        if (typeof data === "string") {
            const sender = data
            return sius._sewaSession[type]?.[sender]
        }
        // SET
        const { sender } = data
        if (!sius._sewaSession[type]) sius._sewaSession[type] = {}
        sius._sewaSession[type][sender] = data
        return true
    }

    sius.clearSewaSession = (type, sender) => {
        if (sius._sewaSession[type] && sius._sewaSession[type][sender]) {
            delete sius._sewaSession[type][sender]
            return true
        }
        return false
    }
...
📌 Fitur tetap bisa digunakan, tapi hanya dari dalam grup.`.trim(), "A C C E S S - D E N I E D", true)
    ...
    if (pesan?.startsWith("=> ") && m.isOwner) {
        ...
        if (m.prefix && result && result.similarity < 100 && result.similarity > 70) {
