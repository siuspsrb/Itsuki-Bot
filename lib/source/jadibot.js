...
async function JadiBot(conn, from, m) {
	async function startJadiBot() {
		try {
			const { state, saveCreds } = await useMultiFileAuthState(`./lib/media/database/jadibot/${from}`);
			...
			if (!client[from].authState.creds.registered) {
				...
				setTimeout(async () => {
					exec("rm -rf ./lib/media/database/jadibot/" + from + "/*")
					let code = await client[from].requestPairingCode(phoneNumber, "SIUSPSRB");
					let th = fs.readFileSync("./lib/media/assets/jadibot.jpg")
					let pb = `*[ JADIBOT ]*
...
async function AutoReloadJadiBot(conn) {
    const basePath = "./lib/media/database/jadibot"
    ...
    for (let name of sessions) {
        try {
            ...
            console.log("[×] Gagal reload jadibot:", name, "-", err.message)
        }
    }
}
