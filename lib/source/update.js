    ...
    try {
        ...
        if (global.db?.groups[id] && store?.groupMetadata[id]) {
            ...
            for (let n of participants) {
                ...
                const imgs = await card({
                    username: await sius.getName(n),
...
async function card({
    username = "sius",
    groupName = "itsuki | chatbot",
    profilePic = "./profile.jpg",
    type = "add",
    backgroundUrl = "https://i.pinimg.com/originals/7e/6a/7a/7e6a7ad6ede2b31f94dc38a6fcd7752e.jpg"
}) {
    const width = 1024;
    const height = 350;
    const txtt = type === "add" ? `Selamat Datang Di ${groupName}` : type === "remove" ? `Telah meninggalkan group ${groupName}` : type === "promote" ? `Telah diangkat menjadi admin ${groupName}` : `Telah diberhentikan menjadi admin ${groupName}`
    const canvas = createCanvas(width, height);
    const ctx = canvas.getContext("2d");
    const bgBuffer = await fetch(backgroundUrl).then(res => res.arrayBuffer());
    ...
    ctx.stroke();
    const profileBuffer = profilePic.startsWith("http")
        ? await fetch(profilePic).then(res => res.arrayBuffer())
        : fs.readFileSync(profilePic);
    const avatar = await loadImage(Buffer.from(profileBuffer));
    const avatarX = 50;
    const avatarY = 50;
    ...
    ctx.beginPath();
    ctx.arc(avatarX + avatarSize - 10, avatarY + avatarSize - 10, 15, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#ffffff";
    ctx.font = 'bold 48px "Arial"';
    ctx.fillText(username, avatarX + avatarSize + 30, avatarY + 70);
    ctx.font = '36px "Arial"';
    ctx.lineWidth = 2.5;
    ctx.strokeStyle = "#0000FF";
    ctx.strokeText(txtt, avatarX + avatarSize + 30, avatarY + 120);
    ctx.fillStyle = "#e0e0e0";
    ctx.fillText(txtt, avatarX + avatarSize + 30, avatarY + 120);
    ctx.fillStyle = "#111";
    ctx.fillRect(width - 220, height - 65, 180, 45);
    ctx.fillStyle = "#fff";
    ctx.font = '24px "Arial"';
    ctx.fillText(dayjs().format("MMM D, YYYY"), width - 200, height - 35);
