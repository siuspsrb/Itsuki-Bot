...
commands.add({
    ...
    run: async({ sius, m, args, Func }) => {
        ...
        if (new Date - user.lastclaim > 86400000) {
            const buf = fs.readFileSync('./lib/media/rpg/daily.jpg')
...
commands.add({
    ...
    run: async({ sius, m, args, Func }) => {
        ...
        if (new Date - user.lasthourly > 3600000) {
            const bufer = fs.readFileSync('./lib/media/rpg/hourly.jpg')
            const cpp = `+ $ 250 Money 💵
+ 1 Wood 🪵
+ 2 Potion 🥤
+ 5 Rock 🪨
...
commands.add({
    ...
    run: async({ sius, m, args, Func }) => {
        ...
        if (new Date - user.lastweekly > 604800000) {
            const bufer = fs.readFileSync('./lib/media/rpg/weekly.jpg')
...
commands.add({
    ...
    run: async({ sius, m, args, Func }) => {
        ...
        if (new Date - user.lastmonthly > 2592000000) {
            const bufer = fs.readFileSync('./lib/media/rpg/monthly.jpg')
