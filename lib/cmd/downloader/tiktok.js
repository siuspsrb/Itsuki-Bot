commands.add({
    ...
    run: async ({ sius, m, args, Func, dl }) => {
        ...
        if (!query) return m.reply(`*[×] Contoh penggunaan:* .tiktoksearch kucing lucu`)
...
commands.add({
    ...
    run: async ({ sius, m, args, Func, dl }) => {
        try {
            if (!args[0]) return m.reply("[×] Harap kirim link TikTok yang ingin didownload!")
            m.reply({ react: { text: "🕣", key: m.key }})
            const result = await dl.tiktok(args[0])
            if (!result || !result.status) {
                return m.reply("[×] Gagal mendownload konten TikTok");
            }
            ...
            if (result.data && result.data[0]?.type.includes("watermark")) {
                ...
                m.reply("[×] Format konten tidak dikenali");
            }
        } catch (err) {
            sius.cantLoad(err)
        }
