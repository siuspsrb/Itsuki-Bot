...
commands.add({
    ...
    run: async ({ sius, m, args, Func }) => {
        ...
        if (!mime) return m.reply(`[×] Kirim atau reply gambar/stiker dengan caption *${m.prefix + m.command} tambahkan pelangi*`)
        ...
        try {
            ...
            if (resultImage) {
                const tempPath = path.join(process.cwd(), "lib/media/sampah", `aiimg_${Date.now()}.png`)
                fs.writeFileSync(tempPath, resultImage)
            if (/image\/webp/.test(mime)) {
                const stickerPath = path.join(process.cwd(), "lib/media/sampah", `aiimg_${Date.now()}.webp`)
            ...
            sius.sendMessage(config.creator, { text: "× Error pak, image edit ai!" }, { quoted: m })
        }
    }
})