...
commands.add({
    ...
    run: async ({ m, args }) => {
        let input = args.join(" ")
        if (!input) return m.reply("× Masukkan deskripsi kode.\n\nContoh: .codegen buat fungsi cek bilangan prima di Go")
        try {
            let result = await pollai(input, {
                model: "gpt-4.1-mini",
            ...
            sius.cantLoad(e)
            await m.reply("× Gagal generate kode. Mungkin server overload.")
        }
    }
})
