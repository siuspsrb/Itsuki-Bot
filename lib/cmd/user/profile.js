import fs from "fs"
import { getPremiumExpired, checkPremiumUser } from "../../source/premium.js"
...
commands.add({
    ...
    run: async ({ sius, m, args, Func }) => {
        ...
        if (args[0] && args[0].toLowerCase() === "setname") {
            ...
            if (user.lastProfile && now < user.lastProfile + 86400000) {
                const timeLeft = Func.clockString(user.lastProfile + 86400000 - now)
        ...
        const profile = await sius.profilePictureUrl(userId, "image").catch(() => "./lib/media/assets/default.jpg")
