import { iGame } from "../../source/game.js"
        ...
        for (const name in games) {
            ...
            if (name === "kuismath") {
                ...
                if (userAns === answer) {
                    ...
                    await m.reply({ sticker: await fs.readFile("./lib/media/assets/benar.webp") })
                    await m.reply(`*+${reward} Point*`)
                    delete game[m.chat + id]
                } else {
                    db.users[m.sender].point -= penalty
                    await m.reply({ sticker: await fs.readFile("./lib/media/assets/salah.webp") })
