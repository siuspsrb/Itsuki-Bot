...
commands.add({
    ...
    run: async ({ sius, m, args }) => {
        try {
            ...
            if (!args[0]) {
                ...
                if (ment === "buttonList") {
                    const menuThumb = fs.readFileSync("./lib/media/assets/allmenu.jpg");
        ...
        },
        {
            ...
            nativeFlowInfo: {
                ...
                paramsJson: JSON.stringify({
                    title: 'List Menu',
                    sections: menuSections // <- ini tetap dynamic
                })
            }
        }
    ],
    ...
    viewOnce: true
            ...
            if (ment === "buttonList" || ment === "documentButtonList") {
                const listThumb = fs.readFileSync("./lib/media/assets/list.jpg");
...
commands.add({
    ...
    run: async ({ sius, m, args }) => {
        try {
            ...
            const allThumb = fs.readFileSync("./lib/media/assets/allmenu.jpg");
            const runtime = `${Math.floor(uptime / 3600)}h ${Math.floor(uptime % 3600 / 60)}m ${Math.floor(uptime % 60)}s`;
